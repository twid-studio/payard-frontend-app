...
export default function HeroPricing() {
  ...
  const filter = useTransform(scrollYProgress, [0.4, 1], ["blur(0vw)", "blur(0.5vw)"]);

  return (
    <motion.section style={{ y, scale, filter }} className={s.hero} ref={heroRef}>
      {data.validFrom && (
        <motion.p {...anim(TitleAnim)} className="small-text shadow">
